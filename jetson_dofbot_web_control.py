...
PICK_VISION_ANGLE_ENABLED = True
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .pick-place { border: 1px solid var(--line); border-radius: 8px; background: #f8fafc; overflow: hidden; }
    ...
    @media (max-width: 620px) { main { width: min(100vw - 20px, 1180px); padding-top: 18px; } h1 { font-size: 24px; } .toolbar { grid-template-columns: 1fr; } .servo { grid-template-columns: 1fr 66px; } .servo input[type="range"] { grid-column: 1 / -1; } .servo .dial { justify-self: end; } }
  </style>
</head>
<body>
  <main>
...
const state = { angles: homeAngles.slice(), connected: false, sendTimer: null, sending: true, pickMode: 'from' };
...
def record_board_cell_pose(key, pose5):
...
def list_pick_locations():
    sources = BOARD_CELL_KEYS + ["layer_4", "layer_3", "layer_2", "layer_1", "yellow", "red", "green", "blue", "blue_2"]
...
def board_cell_scan_pose6(cell_key):
    if cell_key not in BOARD_CELL_KEYS:
        return None
    row = int(cell_key[-2])
    col = int(cell_key[-1])
    for target in FIXED_BOARD_SCAN_TARGETS:
        if target["row"] == row and target["col"] == col:
            return clamp_angles(target["pose"])
    return None

def scan_cell_pick_adjustment_unlocked(cell_key, pick_pose5):
    scan_pose = board_cell_scan_pose6(cell_key)
    if scan_pose is None:
        return pick_pose5[:], {"found": False, "reason": "not a board cell"}, 0

    angles = _move_to_unlocked(scan_pose, ACTIVE_SCAN_MOVE_MS)
    time.sleep(ACTIVE_SCAN_SETTLE_SEC)
    frame = get_fresh_frame(time.time(), timeout=3.0)
    x1, y1, x2, y2 = central_sample_box(frame)
    sample = classify_board_cell(frame, x1, y1, x2, y2)
    detection = detect_block_angle_from_frame(frame)
    detection["scan_angles"] = angles
    detection["cell_sample"] = sample
    adjusted_pose, delta = pose_with_detected_wrist_angle(pick_pose5, detection)
...
def run_pick_place(source_key, target_key):
    ...
    with arm_lock:
        source_pose_used = source["pose"][:]
        target_pose_used = target["pose"][:]

        if source_key in BOARD_CELL_KEYS:
            source_pose_used, scan_detection, scan_delta = scan_cell_pick_adjustment_unlocked(source_key, source_pose_used)
            steps.append({"name": "scan_source_before_pick", "detection": scan_detection})
        ...
        steps.append({"name": "grip", "angles": current_angles[:]})
        ...
        recorded_target = record_board_cell_pose(target_key, target_pose_used)
        if recorded_target:
...
def run_center_grab_test():
    ...
    with arm_lock:
        ...
        steps.append({"name": "at_center", "angles": angles})
...
def run_center_pick_place_cycle():
    steps = []
    with arm_lock:
        center_pick_pose = PICK_CENTER_POSE_5[:]
        center_pick_pose, scan_detection, wrist_delta = scan_cell_pick_adjustment_unlocked("cell_22", center_pick_pose)
        steps.append({"name": "scan_center_before_pick", "detection": scan_detection})
        ...
        angles = _move_pick_pose_array_unlocked(center_pick_pose, 1000, GRIPPER_OPEN)
        ...
        angles = _move_pick_pose_array_unlocked(center_pick_pose, 1000, GRIPPER_CLOSE)
        steps.append({"name": "at_center_place", "angles": angles})

        _set_gripper_unlocked(GRIPPER_OPEN, 600)
        steps.append({"name": "release_center", "angles": current_angles[:]})

        recorded_target = record_board_cell_pose("cell_22", center_pick_pose)
        if recorded_target:
            steps.append({"name": "record_center_pose", "target": recorded_target})
...
def detect_pick_pose_adjustment_unlocked(pose5):
    ...
    frame = get_fresh_frame(time.time(), timeout=3.0)
    detection = detect_block_angle_from_frame(frame)
    adjusted_pose, delta = pose_with_detected_wrist_angle(pose5, detection)
    detection["wrist_delta"] = delta
    detection["adjusted_pose"] = adjusted_pose
    return adjusted_pose, detection, delta
