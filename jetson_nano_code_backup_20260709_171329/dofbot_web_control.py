...
def board_cell_place_pose5(key):
    ...
    remembered = item.get("last_pick_pose")
    if key in BOARD_CELL_KEYS and isinstance(remembered, list) and len(remembered) == 5:
        pose = clamp_pick_pose5(remembered)
    else:
        pose = clamp_pick_pose5(item["pose"][:])
    offset = BOARD_PLACE_POSE_OFFSETS.get(key)
    if isinstance(offset, list) and len(offset) == 5:
        pose = clamp_pick_pose5([pose[i] + offset[i] for i in range(5)])
    return pose

def board_cell_pick_pose5(key):
    item = PICK_LOCATIONS[key]
    placed = item.get("last_place_pose")
    if key == "cell_22" and isinstance(placed, list) and len(placed) == 5:
        return clamp_pick_pose5(placed)
    remembered = item.get("last_pick_pose")
    if key in BOARD_CELL_KEYS and isinstance(remembered, list) and len(remembered) == 5:
        return clamp_pick_pose5(remembered)
    return clamp_pick_pose5(item["pose"][:])
...
def record_board_cell_pose(key, pose5, place_angles6=None):
    ...
    if not PICK_LOCATIONS[key].get("last_pick_pose"):
